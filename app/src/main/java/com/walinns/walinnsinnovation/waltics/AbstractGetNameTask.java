...
public abstract class AbstractGetNameTask extends AsyncTask<Void, Void, Void> {
   ...
   public static String GOOGLE_USER_DATA="No_data";
	...
	AbstractGetNameTask(MainActivity activity, String email, String scope) {
		...
		sharedCommon = new SharedCommon(activity);
		this.mScope = scope;
		this.mEmail = email;
	}
	...
	private void fetchNameFromProfileServer() throws IOException, JSONException {
		...
		System.out.println("Googgle user data etey : "+ token);
		WalinnsAPI.getInstance().pushProfile(token);
		...
		if (sc == 200) {
			...
			System.out.println("Googgle user data : "+ GOOGLE_USER_DATA);
			JSONObject profileData = new JSONObject(
					AbstractGetNameTask.GOOGLE_USER_DATA);
				WalinnsAPI.getInstance().pushProfile(token);
				Intent intent = new Intent(mActivity, HomeScreen.class);
				if(profileData.has("name")) {
					sharedCommon.save(SharedCommon.email, profileData.getString("name"));
					intent.putExtra("Email", profileData.getString("name"));
				}else {
